...
void removeUtil(node n)
{
	...
	else if (n->parent->color == RED && s->color == BLACK &&
		s->left->color == BLACK && s->right->color == BLACK)
	{
		s->color = RED;
	...
	{
		if (n == n->parent->left && s->color == BLACK &&
		...
		else if (n == n->parent->right && s->color == BLACK &&
			s->right->color == RED && s->left->color == BLACK)
		{
			s->color = RED;
			s->right->color = BLACK;
			rotateLeft(s);
...
{
	node ret = NULL;
	for (node p = *t; p != NULL; ret = p, p = (p->value > v ? p->left : p->right));
	return ret;
}
void printNode(node n)
{
